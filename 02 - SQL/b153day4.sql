...
--calisanlar3 tablosunda ikinci en yüksek maaşı alanları gösteriniz
...
FROM nisan
WHERE EXISTS (SELECT urun_isim FROM mart WHERE mart.urun_isim=nisan.urun_isim )--daha hızlı çalışır

SELECT urun_isim,musteri_isim 
FROM nisan 
WHERE urun_isim IN ( SELECT urun_isim from mart);--performansı daha yavaş
